...
EnemyRangedDroneElite::EnemyRangedDroneElite(Player *player, QGraphicsScene *scene, QObject *parent)
    : EnemyRangedBase{player, scene, parent}, attack_max(600)
{
    pix = QPixmap(":/source/picture/enemy/en_float_elite.png");
    setPixmap(pix);
    setScale(6.5);
    setTransformOriginPoint(boundingRect().center());
    attack_count = 0;
    connect(e_timer, &QTimer::timeout, this, &EnemyRangedDroneElite::attack);
...
void EnemyRangedDroneElite::attack()
{
    ...
    angle -= 15;
    for (int i = 0; i < 3; ++i) {
        BulletEnemy *bullet = new BulletEnemy(pos(), angle + 15*i, 3, 1, 0, e_scene, e_player, e_scene);
        bullet->setScale(2);
